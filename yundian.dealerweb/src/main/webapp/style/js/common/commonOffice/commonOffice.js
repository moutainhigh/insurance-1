 ...
 */
var customerId,businessTypeCode,paymentMethod;
...
function countDownPayment(){
    if(!isNaN($("#billingPrice").val())&&!isNaN($("#loanAmount").val())){
        $("#downPaymentAmount").val(parseFloat($("#billingPrice").val() || "0")-parseFloat($("#loanAmount").val() || "0"));
    }
...
function countLoanRatio(){
    if(!isNaN($("#loanAmount").val())&&!isNaN($("#billingPrice").val())&&parseFloat($("#billingPrice").val() || "0")>0){
        $("#loanRatio").val(((parseFloat($("#loanAmount").val() || "0")/parseFloat($("#billingPrice").val()))*100).toFixed(2));
    }
}
//计算预收金额
...
var feeArr=["insuranceAmount","accountDeposit","renewalDeposit","pbDeposit","costAmount","purchaseTax","guarantyRiskAmount","serviceFee","gpsInstallationFee","liabilityAmount","agencyFee1","agencyFee2","otherFee","bankIrsFee","licenseFee","doorSurveyFee","agencyFee","outsourceSurveyFee"];
//计算费用合计
function countTotalFee(){
    var feeArr3=["insuranceAmount","accountDeposit","renewalDeposit","pbDeposit","costAmount","purchaseTax","guarantyRiskAmount","serviceFee","gpsInstallationFee","liabilityAmount","bankIrsFee","licenseFee","doorSurveyFee","agencyFee","outsourceSurveyFee"];
    var totalFee = 0;
...
function counttestedRepaymentAmount(){
    ...
    if(!isNaN($("#loanAmount").val())&&!isNaN($("[name='handingFee']").val())){
        var fee=((parseFloat($("#loanAmount").val()|| "0")*(1+(parseFloat($("[name='handingFee']").val()|| "0")/100)))/qs).toFixed(2);
    ...
    comn.ajax({
        ...
        success: function (res) {
            ...
            if(res.data.maritalStatus==1){
                ...
                $("#province_3").getProvince(res.data.spouseCompanyAddressPid).change(function() {
                    if (this.value) {
            ...
            $("#province_1").getProvince(res.data.homeAddressPid).change(function() {
                if (this.value) {
            ...
            $("#province_2").getProvince(res.data.companyAddressPid).change(function() {
                if (this.value) {
