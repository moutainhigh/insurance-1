...
var allImg = window.parent.allImg;
var imgIds = window.parent.imgIds;
...
$.fn.extend({ 
	photoGallery: function(options) { 
			...
			//右下角缩略图高度
 	 	 	...
 	 	 	template : {
	 	 	 	...
	 	 	 	OPERTATION : '<div class="oper">' +
								'<span class="prev icon_tool-prev"></span>' +
								'<span class="next icon_tool-next"></span>' +
							 '</div>' +
							 '<div class="tool">' +
							 	'<div class="toolct">' +
		...
		var o = $.extend(defaults, options), $gallery = $(this);
		...
		$prev.on('click',function(){
			...
			if(allImg[o.activeIndex] && allImg[o.activeIndex].hasRead == 1 && imgIds.indexOf(allImg[o.activeIndex].id) == -1){ 
				imgIds.push(allImg[o.activeIndex].id);
				console.log(imgIds);
			}
			//window.parent.toVal(allImg[o.activeIndex].dirId); 
			window.parent.curImg(parseInt(o.activeIndex) + 1);
  			toggleImage();	
	  		...
	  		if (e.keyCode) {
			   	if(e.keyCode == 37 ){ //left
					...
					if(allImg[o.activeIndex] && allImg[o.activeIndex].hasRead == 1 && imgIds.indexOf(allImg[o.activeIndex].id) == -1){
			  		...
			  		toggleImage();	
					window.parent.curImg(o.activeIndex + 1)
			   	}
...
var imgArr = [];
function loadImg(list, index, callback) {
	var wH = 415,
		wW = 615,
		imgHeight,
		imgWidth,
		...
		maxHeight,
		maxWidth;
		if(list.length == 0) { return imgArr; }
		var image = new Image();
		image.src = list[0];
		...
		if(index != null){
		}else{ imgUrl = list[index - 1]; }

		if(url == imgUrl){ //加载第一张图片 
			activeIndex = index;
			w = imgWidth;
			h = imgHeight;
		} 
		...
		list.splice($.inArray(url, list),1);
		loadImg(list, null, callback);
		if(list.length == 0 && typeof(callback) == "function"){ callback(imgArr) }
	};
}

$.extend({
	//打开图片查看器
	openPhotoGallery : function(imgList, index){
		imgArr = [];
		window.parent.totalImg(imgList.length);
		window.parent.curImg(index + 1);
		//var mask = layer.load(); //开始加载图片 
		$("#imgIframe").html("");
		loadImg(imgList, index, function(imgs){
			//layer.close(mask);
			localStorage["photoGalleryImgs"] = JSON.stringify(imgs);
			localStorage["photoGalleryActiveIndex"] = activeIndex; 
			
			$("#J_pg").remove();
			$("<iframe></iframe>").appendTo("#imgIframe")
				.attr("id", "J_pg")
				.attr("src", "./common/plugs/gallery/iframe/index.html")
				.css({
					position : "absolute",
					top : "0",
					left: "0",
