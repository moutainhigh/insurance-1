...
class LoanListAddModal extends Component {
  ...
  handleOnAuditOK = (e) => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      console.log("handleOnAuditOK:"+JSON.stringify(values))
          if (!err) {
            LoanListAction.auditOK(values);
      }
    });
  }
//拒绝
  handleOnReject = (e) => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      console.log("handleOnReject:"+JSON.stringify(values))
      if (!err) {
        LoanListAction.auditReject(values);
      }
    });
  }
  //退回
  handleOnReturn = (e) => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      console.log("handleOnReturn:"+JSON.stringify(values))
      if (!err) {
        LoanListAction.auditReturn(values);
      }
    });
  }
      ...
      <div>
        <Modal width={1000} visible={this.props.addModalVisible} title="保险分期" onCancel={this.handleAddCancel}
               footer={[
                 ...
                 <Button key="submitTJ" type="primary" size="large" onClick={this.handleOnAuditOK}>审核通过</Button>,
                 <Button key="submit" type="primary" size="large" onClick={this.handleOnReject}>拒绝</Button>,
                 <Button key="submit" type="primary" size="large" onClick={this.handleOnReturn}>退回</Button>,
                 <Button key="back" type="ghost" size="large" onClick={this.handleAddCancel}>返回</Button>,
               ]}>
...
LoanListAddModal = Form.create()(LoanListAddModal);
